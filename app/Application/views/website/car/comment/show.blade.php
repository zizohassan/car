            ...
            <li>
                <div>
                    ...
                    @if(auth()->check() && $comment->user_id == auth()->user()->id)
                        @if(auth()->check()->group_id == 1)
                            <a
                                href="{{concatenateLangToUrl("car/delete/comment/".$comment->id)}}"
                                class="btn btn-danger"><i class="fa fa-trash"></i></a>
                        @endif
                    <span class="btn btn-info" onclick="$(this).next().slideToggle()"><i class="fa fa-edit"></i></span>
                    <form method="post" action="{{ concatenateLangToUrl("car/update/comment/".$comment->id) }}"
