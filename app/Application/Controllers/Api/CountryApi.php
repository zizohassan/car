...
use App\Application\Transformers\CountryTransformers;
use App\Application\Requests\Website\Country\ApiAddRequestCountry;
use App\Application\Requests\Website\Country\ApiUpdateRequestCountry;
...
class CountryApi extends Controller
{
    ...
    public function index($limit = 10 , $offset = 0 , $lang = "en"){
       ...
       if($data){
             return response(apiReturn(CountryTransformers::transform($data))  , 200 );
       }
    ...
    public function add(ApiAddRequestCountry $validation){
    ...
    public function update($id , ApiUpdateRequestCountry $validation){
        $request = $this->checkRequestType();
         $v = Validator::make($this->request->all(), $validation->rules());
         if ($v->fails()) {
            return response(apiReturn('' , 'error' , $v->errors())  , 401 );
