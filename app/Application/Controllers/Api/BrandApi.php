...
use App\Application\Transformers\BrandTransformers;
use App\Application\Requests\Website\Brand\ApiAddRequestBrand;
use App\Application\Requests\Website\Brand\ApiUpdateRequestBrand;
...
class BrandApi extends Controller
{
    ...
    public function __construct(Brand $model , Request $request)
    {
        $this->model = $model;
    ...
    public function index($limit = 10 , $offset = 0 , $lang = "en"){
       ...
       if($data){
             return response(apiReturn(BrandTransformers::transform($data))  , 200 );
       }
    ...
    public function add(ApiAddRequestBrand $validation){
    ...
    public function update($id , ApiUpdateRequestBrand $validation){
        $request = $this->checkRequestType();
         $v = Validator::make($this->request->all(), $validation->rules());
         if ($v->fails()) {
            return response(apiReturn('' , 'error' , $v->errors())  , 401 );
