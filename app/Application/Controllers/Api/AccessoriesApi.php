...
use App\Application\Transformers\AccessoriesTransformers;
use App\Application\Requests\Website\Accessories\ApiAddRequestAccessories;
use App\Application\Requests\Website\Accessories\ApiUpdateRequestAccessories;

class AccessoriesApi extends Controller
{

    protected $request;
    protected $model;

    public function __construct(Accessories $model , Request $request)
    {
        $this->model = $model;
    ...
    public function index($limit = 10 , $offset = 0 , $lang = "en"){
       ...
       if($data){
             return response(apiReturn(AccessoriesTransformers::transform($data))  , 200 );
       }
    ...
    public function add(ApiAddRequestAccessories $validation){
    ...
    public function update($id , ApiUpdateRequestAccessories $validation){
        $request = $this->checkRequestType();
         $v = Validator::make($this->request->all(), $validation->rules());
         if ($v->fails()) {
            return response(apiReturn('' , 'error' , $v->errors())  , 401 );
